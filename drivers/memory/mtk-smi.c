...
/*
 * MT6789 / MT8781 LARB outstanding tables.
 *
 * DERIVED verbatim from the vendor MT8781 kernel,
 * drivers/memory/mtk-smi.c::mtk_smi_larb_mt6789_bwl[][], which the vendor
 * driver writes to SMI_LARB_OSTDL_PORTx - the same register mainline's
 * .ostd writes. Empty rows are larbs that do not exist on this SoC.
 */
 ...
 * MT6789 / MT8781.
 *
 * larb_direct_to_common_mask: DERIVED - copied verbatim from the vendor
 * MT8781 kernel's mtk_smi_larb_mt6789.larb_direct_to_common_mask. Those larb
 * ids are masters wired straight to the SMI common, so config_port must skip
 * them. (Note larb19 is in this mask even though the port header names larb19
 * ports; that is the vendor's own data, kept as-is.)
 *
 * flags_general: DERIVED - the vendor's mtk_smi_larb_mt6789_misc[] programs,
 * for every larb, SMI_LARB_CMD_THRT_CON = 0x370256 and SMI_LARB_SW_FLAG = 0x1.
 * 0x370256 has bits[7:4] == 0x5, which is exactly what mainline's
 * MTK_SMI_FLAG_THRT_UPDATE writes (SMI_LARB_THRT_RD_NU_LMT == 5 << 4), and
 * SW_FLAG = 1 is MTK_SMI_FLAG_SW_FLAG (SMI_LARB_SW_FLAG_1). The vendor tables
 * for MT6893 are identical here and mainline's mt6893 entry sets the same two
 * flags.
 *
 * MTK_SMI_FLAG_SLEEP_CTL is deliberately NOT set: the vendor mt6789 larb_gen
 * has no .sleep_ctrl callback (unlike the SoCs that need it).
 */
static const struct mtk_smi_larb_gen mtk_smi_larb_mt6789 = {
	.config_port                = mtk_smi_larb_config_port_gen2_general,
	.flags_general	            = MTK_SMI_FLAG_THRT_UPDATE | MTK_SMI_FLAG_SW_FLAG,
	.larb_direct_to_common_mask = BIT(3) | BIT(5) | BIT(6) | BIT(8) |
				      BIT(10) | BIT(11) | BIT(12) | BIT(15) |
				      BIT(18) | BIT(19) | BIT(21) | BIT(22),
...
/*
 * MT6789 / MT8781 SMI common init.
 *
 * DERIVED from the vendor MT8781 kernel's mtk_smi_common_mt6789_misc[0]:
 *	{L1LEN 0xb} {BUS_SEL 0x4514} {M4U_TH 0xe100e10} {FIFO_TH1 0x90a090a}
 *	{FIFO_TH2 0x506090a} {DCM 0x4f1} {DUMMY 0x1}
 * BUS_SEL is expressed through .bus_sel below instead (see cross-check there),
 * leaving exactly the six pairs mainline's SMI_COMMON_INIT_REGS_NR allows.
 */
 ...
 * MT6789 / MT8781.
 *
 * bus_sel: DERIVED and cross-checked two ways. The vendor MT8781 kernel's
 * mtk_smi_common_mt6789 uses F_MMU1_LARB(1|2|4|5|7), and its raw register
 * table writes SMI_BUS_SEL = 0x4514, which is bit-for-bit the same value
 * (F_MMU1_LARB(larbid) == BIT(larbid * 2)).
 *
 * has_gals: DERIVED - vendor sets .has_gals = true, and every MT6789 SMI
 * common/sub-common node in the vendor DT has "gals0"/"gals1" clocks.
 */
